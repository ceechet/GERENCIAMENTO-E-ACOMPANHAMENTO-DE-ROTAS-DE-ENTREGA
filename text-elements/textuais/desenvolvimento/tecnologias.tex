...
Paralelamente a isso, segundo \citeonline{tulach2008practical}, API é um conjunto de classes, seus métodos e seus campos.
...
\newpage
\subsection{REST}
%Porém, a mesma não define tecnologias que devem ser utilizadas
Segundo \citeonline{rest:roy}, REST é uma sigla que significa \textit{Representational State Transfer}, é um modelo arquitetural que define um conjunto de regras simples para implementar serviços web, sem, no entanto, especificar as tecnologias que devem ser utilizadas. Uma forma de implementar REST é utilizar o conceito de \textit{RESTful} que se baseia no protocolo da aplicação HTTP, no padrão de nomenclatura URI e na linguagem XML/JSON. As respostas do REST são cacheáveis, o que possibilita um grande aumento de \textit{performance} para clientes simples. Utilizando esse conceito, os serviços tornam-se disponíveis para diferentes aplicações clientes, ou seja, não há necessidade de mudança de uma aplicação para outra, pois o serviço concentra-se em um servidor próprio. 
...
\citeonline{rest:roy} destaca os seguintes pontos sobre o REST:
\begin{itemize}
...
\begin{quadro}[H]
    ...
    \label{quadro:propertycoords}}
\begin{tabular}{|l|l|}
\hline
\textbf{Propriedade} & \textbf{Detalhes} \\ \hline
coords.latitude & Número decimal em graus da latitude \\ \hline
coords.longitude & Número decimal em graus da longitude \\ \hline
coords.altitude & Altura em metros da posição abaixo da Elipsoide de referência \\ \hline
coords.accuracy & \begin{tabular}[c]{@{}l@{}}A precisão em metros do resultado retornado. O valor dessa \\ configuração informa à aplicação o quão útil é o valor retornado\\ para latitude/longitude. Isso pode ajudar a determinar \\ se o resultado retornado é preciso o suficiente para o propósito\\ a que se destina\end{tabular} \\ \hline
coords.altitudeAccuracy & A precisão em metros da altitude retornada \\ \hline
coords.heading & \begin{tabular}[c]{@{}l@{}}Direção que o dispositivo deve tomar, no sentido horário a\\ partir do norte\end{tabular} \\ \hline
coords.speed & A velocidade de solo atual do dispositivo em metros por segundo \\ \hline
timestamp & \textit{Timestamp} de quando a posição foi adquirida \\ \hline
\end{tabular}
