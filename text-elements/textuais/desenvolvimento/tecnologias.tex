...
Para que seja possível capturar a posição enquanto o objeto ou usuário estiver com sua posição em modo de alteração, como movimentação do dispositivo, é necessário utilizar o método \textit{watchPosition}. A \autoref{tab:propertycoords} apresenta as propriedades presentes no objeto de retorno da função. Entre estes, apenas \textit{coords.latitude} e \textit{coords.longitude} são garantidos para serem devolvidos (todos os outros podem ser nulos) \cite{geolocalizacao:2011}.

\begin{table}[H]
    \centering
    \caption{Propriedades dos métodos de geolocalização
    \label{tab:propertycoords}}
\begin{tabular}{|l|l|}
\hline
...
coords.accuracy & \begin{tabular}[c]{@{}l@{}}A precisão em metros do resultado retornado. O valor dessa \\ configuração informa a aplicação quão útil é o valor retornado\\ latitude/longitude é realmente. Isso pode ajudar a determinar \\ se o resultado retornado é precisa o suficiente para o propósito\\ a que se destina\end{tabular} \\ \hline
...
coords.speed & A velocidade desolo atual do dispositivo em metros por segundo \\ \hline
timestamp & Timestamp de quando a posição foi adquirida \\ \hline
\end{tabular}
