...
%%% Rev-Madalozzo: "pedidos gerados nos estabelicmentos"- Q estabelecimentos? Em algum lugar no texto, antes deste ponto, você comenta sobre os estabelecimentos? Quem são eles? Como eles vão se comunicar contigo? Etc.
%%% Rev-Madalozzo: "entregas" - Que tipo de entregas? Entregas de comida? Entregas de frete? Falou sobre isso antes em algum lugar e que eu tenha perdido a leitura?

O presente trabalho apresenta uma API REST, a qual possibilita a coleta das coordenadas geográficas dos pedidos gerados nos estabelecimentos. Além disso, apresenta o desenvolvimento de uma aplicação para gerenciar, por meio de um painel de controle, a logística das entregas. Tal aplicação tem por objetivo fornecer ao motoboy um módulo de entregas, possibilitando a ele observar no mapa a rota e os pontos de parada que devem ser percorridos. Todo o processo de comunicação da aplicação ocorre totalmente em tempo real, o que possibilita agilidade e segurança quanto ao destino final.
...
\begin{figure}[H]
    ...
    \includegraphics[width=0.7\textwidth]{./dados/figuras/fig6}
    \fonte{Autor}
...
%%% Rev-Madalozzo: Migration: De um exemplo de criação de uma tabela simples e um com FK que você tenha criado. Explica como se cria (linha de comando artisan) uma migration e como se executa ela. E a importância principal de trabalhar com migrations, versionamento da base de dados. 
